...
Процедура ПослеЗаписи(ПараметрыЗаписи)
	...
	Если ПараметрыЗаписи.РежимЗаписи = РежимЗаписиДокумента.Проведение Тогда
		ОценкаПроизводительностиКлиент.УстановитьКлючевуюОперациюЗамера(ИдентификаторЗамераПроведениеНеНужнаРегистрацияОшибки, "ПроведениеДокументаПрисвоениеВоинскогоСпециальногоЗванияСписком");
	КонецЕсли;
	
	Оповестить("Запись_ПрисвоениеВоинскогоСпециальногоЗванияСписком", ПараметрыЗаписи, Объект.Ссылка);
...
Процедура ПодборСотрудников(Команда)
	...
	ОценкаПроизводительностиКлиент.НачатьЗамерВремени(Истина, "ПодборСотрудникаВФормеДокументаПрисвоениеВоинскогоСпециальногоЗванияСписком");
...
Процедура ДанныеВРеквизит()
	...
	ТекущиеЗначенияСовокупныхТарифныхСтавок = Документы.ПрисвоениеВоинскогоСпециальногоЗванияСписком.ТекущиеЗначенияСовокупныхТарифныхСтавокСотрудников(
...
Процедура РеквизитВДанные(ТекущийОбъект)
	...
	Для каждого ПоказателиСотрудника Из ЭтаФорма["ПоказателиСотрудников"] Цикл
		...
		Строка.ВоинскоеСпециальноеЗвание = ПоказателиСотрудника.ВоинскоеСпециальноеЗвание;
	КонецЦикла;
...
Процедура ЗаполнитьПоказатель()
	...
	Объект.Показатель = ВоинскиеСпециальныеЗвания.ПоказательОкладПоВоинскомуСпециальномуЗванию();
...
Процедура ДополнитьПоказателиСотрудников(МассивСотрудников)
	...
	ТаблицаНачисленийСотрудников = Документы.ПрисвоениеВоинскогоСпециальногоЗванияСписком.НачисленияСотрудников(
	...
	Для каждого Сотрудник Из МассивСотрудников Цикл
		...
		Иначе
			...
			НоваяСтрока.ВоинскоеСпециальноеЗвание = Справочники.ВоинскиеСпециальныеЗвания.ПустаяСсылка();
	...
	ПоляОтбора = Новый Структура("ВоинскиеСпециальныеЗванияФизическихЛиц", Отбор);
	
	КадровыеДанные = КадровыйУчет.КадровыеДанныеСотрудников(Ложь, МассивСотрудников, "ВоинскоеСпециальноеЗвание", Объект.ДатаПрисвоения, ПоляОтбора, Ложь);  
	...
	Для Каждого КадровыеДанныеСотрудника Из КадровыеДанные Цикл
		...
		СтрокаСотрудника.ВоинскоеСпециальноеЗвание = КадровыеДанныеСотрудника.ВоинскоеСпециальноеЗвание;
		
		Если ЗначениеЗаполнено(СтрокаСотрудника.Сотрудник) И ЗначениеЗаполнено(Объект.ДатаПрисвоения) И ЗначениеЗаполнено(СтрокаСотрудника.ВоинскоеСпециальноеЗвание) Тогда 
			СтрокаСотрудника.ЗначениеПоказателя = ВоинскиеСпециальныеЗвания.ЗначениеОкладаПоВоинскомуСпециальномуЗванию(СтрокаСотрудника.ВоинскоеСпециальноеЗвание, Объект.ДатаПрисвоения);
		Иначе
	...
	Менеджер = Документы.ПрисвоениеВоинскогоСпециальногоЗванияСписком;
...
&НаСервере
Процедура ПоказателиСотрудниковВоинскоеСпециальноеЗваниеПриИзмененииНаСервере(Идентификатор)
	...
	Если НЕ НайденныеСтроки.Количество() = 0 Тогда		
		НайденныеСтроки[0].ВоинскоеСпециальноеЗвание = ТекущиеДанные.ВоинскоеСпециальноеЗвание;
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(ТекущиеДанные.Сотрудник) И ЗначениеЗаполнено(Объект.ДатаПрисвоения) И ЗначениеЗаполнено(ТекущиеДанные.ВоинскоеСпециальноеЗвание) Тогда 
		ТекущиеДанные.ЗначениеПоказателя = ВоинскиеСпециальныеЗвания.ЗначениеОкладаПоВоинскомуСпециальномуЗванию(ТекущиеДанные.ВоинскоеСпециальноеЗвание, ТекущиеДанные.ВремяРегистрации);
...
&НаКлиенте
Процедура ПоказателиСотрудниковВоинскоеСпециальноеЗваниеПриИзменении(Элемент)
	ПоказателиСотрудниковВоинскоеСпециальноеЗваниеПриИзмененииНаСервере(Элементы.ПоказателиСотрудников.ТекущаяСтрока);
КонецПроцедуры
