...
Процедура ПослеЗаписи(ПараметрыЗаписи)
	Оповестить("Запись_ОтветФСКН", ПараметрыЗаписи, Объект.Ссылка);
КонецПроцедуры
...
Процедура СотрудникиОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Для Каждого Сотрудник Из ВыбранноеЗначение Цикл
		Если Объект.Сотрудники.НайтиСтроки(Новый Структура("Сотрудник", Сотрудник)).Количество() = 0 Тогда
			...
			СтрокаТаблицы.Состояние = ПредопределенноеЗначение("Перечисление.СостоянияСогласования.Согласовано");
		КонецЕсли;	
	КонецЦикла;	
...
Процедура УстановитьДоступностьЭлементов(Форма)
	...
	Форма.Элементы.ОрганФСКН.Доступность	= НЕ ДокументОснованиеВыбран;
...
Процедура ЗаполнитьНаСервере()
	...
	Если ТекущийОбъект.МожноЗаполнитьПоЗапросуВФСКН() Тогда
		ТекущийОбъект.ЗаполнитьПоЗапросуВФСКН();
...
Функция СписокСотрудников()
	...
	Запрос.Текст = 
		...
		|ГДЕ
		|	ЗапросВФСКНСотрудники.Ссылка = &ДокументОснование";
...
Функция КлючевыеРеквизитыЗаполненияФормыОписаниеКлючевыхРеквизитов() Экспорт
	...
	Массив.Добавить(Новый Структура("ЭлементФормы, Представление", "ДокументОснование",	Нстр("ru = 'запроса ФСКН'")));
