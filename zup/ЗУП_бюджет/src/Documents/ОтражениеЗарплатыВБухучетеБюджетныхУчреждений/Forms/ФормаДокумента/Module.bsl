...
Процедура ПослеЗаписи(ПараметрыЗаписи)
	Оповестить("Запись_ОтражениеЗарплатыВБухучетеБюджетныхУчреждений", ПараметрыЗаписи, Объект.Ссылка);
КонецПроцедуры
...
Процедура ОбновитьДоступностьЗарплатаОтраженаВБухучете()
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.ОбменЗГУБГУ1") Тогда
		...
		ОбменИспользуется = ОбменИспользуется Или Модуль.ОбменИспользуется(Объект.Организация);
	КонецЕсли;
...
Функция РезультатЗаполненияВДлительнойОперации()
	...
	СтруктураПараметров.ИсточникФинансовогоОбеспечения = Объект.ИсточникФинансовогоОбеспечения;
	
	НаименованиеЗадания = НСтр("ru = 'Заполнение документа ""Отражение зарплаты в бухгалтерском учете""'");
	
	Результат = ДлительныеОперации.ЗапуститьВыполнениеВФоне(
		УникальныйИдентификатор,
		"Документы.ОтражениеЗарплатыВБухучетеБюджетныхУчреждений.ПодготовитьДанныеДляЗаполнения",
		СтруктураПараметров,
		НаименованиеЗадания);
...
Процедура ПровестиДокумент()
	...
	Если Результат.ЗаданиеВыполнено Тогда
		...
		Оповестить("Запись_ОтражениеЗарплатыВБухучетеБюджетныхУчреждений", ПараметрыОповещения, Объект.Ссылка);
...
Функция РезультатПроведенияВДлительнойОперации()
	...
	Результат = ДлительныеОперации.ЗапуститьВыполнениеВФоне(
		УникальныйИдентификатор,
		"Документы.ОтражениеЗарплатыВБухучетеБюджетныхУчреждений.ВыполнитьПроведение",
		СтруктураПараметров,
		НаименованиеЗадания);
