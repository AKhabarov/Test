...
Процедура ПослеЗаписи(ПараметрыЗаписи)
	Оповестить("Запись_ПрисвоениеВоинскогоСпециальногоЗвания", ПараметрыЗаписи, Объект.Ссылка);
КонецПроцедуры
...
&НаКлиенте
Процедура ВоинскоеСпециальноеЗваниеПриИзменении(Элемент)
	
	ВоинскоеСпециальноеЗваниеПриИзмененииНаСервере();
...
Процедура ПрочитатьКадровыеДанныеСотрудника()
	...
	КадровыеДанные = КадровыйУчет.КадровыеДанныеСотрудников(Истина, Объект.Сотрудник, "КоличествоСтавок", Объект.ДатаПрисвоения);
...
Процедура УстановитьТекущиеДанныеСотрудника()
	...
	Документы.ПрисвоениеВоинскогоСпециальногоЗвания.ЗаполнитьНачисленияСотрудника(Объект, ВремяРегистрации);
	
	ПрочитатьКадровыеДанныеСотрудника();
	ЗаполнитьВоинскоеСпециальноеЗваниеСотрудника();
...
&НаСервере
Процедура ЗаполнитьВоинскоеСпециальноеЗваниеСотрудника()
	
	УстановитьПривилегированныйРежим(Истина);
	
	Объект.ВоинскоеСпециальноеЗвание = Неопределено;
	...
	КадровыеДанные = КадровыйУчет.КадровыеДанныеСотрудников(Ложь, СписокСотрудников, "ВоинскоеСпециальноеЗвание", ВремяРегистрации, ПоляОтбора, Ложь);  
	
	Если КадровыеДанные.Количество() > 0 Тогда 
		Объект.ВоинскоеСпециальноеЗвание = КадровыеДанные[0].ВоинскоеСпециальноеЗвание;
	КонецЕсли;
...
Процедура ЗаполнитьЗначениеПоказателя()
	...
	Если Не ЗначениеЗаполнено(Объект.Сотрудник) Или Не ЗначениеЗаполнено(Объект.ДатаПрисвоения) Или Не ЗначениеЗаполнено(Объект.ВоинскоеСпециальноеЗвание) Тогда 
		Возврат;
	КонецЕсли;
	
	Объект.ЗначениеПоказателя = ВоинскиеСпециальныеЗвания.ЗначениеОкладаПоВоинскомуСпециальномуЗванию(Объект.ВоинскоеСпециальноеЗвание, ВремяРегистрации);
...
&НаСервере
Процедура ВоинскоеСпециальноеЗваниеПриИзмененииНаСервере()
...
Процедура РассчитатьФОТПоДокументу()
	...
	Документы.ПрисвоениеВоинскогоСпециальногоЗвания.РассчитатьФОТПоДокументу(Объект, ВремяРегистрации);
