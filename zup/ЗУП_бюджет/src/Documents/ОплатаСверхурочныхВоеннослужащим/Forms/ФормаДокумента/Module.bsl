...
Процедура ПослеЗаписи(ПараметрыЗаписи)
	...
	Если ПараметрыЗаписи.РежимЗаписи = РежимЗаписиДокумента.Проведение Тогда
		ОценкаПроизводительностиКлиент.УстановитьКлючевуюОперациюЗамера(ИдентификаторЗамераПроведениеНеНужнаРегистрацияОшибки, "ПроведениеДокументаОплатаСверхурочныхВоеннослужащим");
	КонецЕсли;
	...
	Оповестить("Запись_ОплатаСверхурочныхВоеннослужащим", ПараметрыЗаписи, Объект.Ссылка);
...
Процедура НачисленияВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	//РедактированиеДенежногоДовольствия(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка);
...
Процедура Подключаемый_Исправить(Команда)
	ИсправлениеДокументовЗарплатаКадрыКлиент.Исправить(Объект.Ссылка, "ОплатаСверхурочныхВоеннослужащим");
КонецПроцедуры
...
Процедура Подключаемый_ПерейтиКИсправлению(Команда)
	ИсправлениеДокументовЗарплатаКадрыКлиент.ПерейтиКИсправлению(ЭтаФорма.ДокументИсправление, "ОплатаСверхурочныхВоеннослужащим");
КонецПроцедуры

&НаКлиенте
Процедура Подключаемый_ПерейтиКИсправленному(Команда)
	ИсправлениеДокументовЗарплатаКадрыКлиент.ПерейтиКИсправленному(Объект.ИсправленныйДокумент, "ОплатаСверхурочныхВоеннослужащим");
КонецПроцедуры
...
Процедура ПодборСотрудников(Команда)
	...
	ОценкаПроизводительностиКлиент.НачатьЗамерВремени(Истина, "ПодборСотрудникаВФормеДокументаОплатаСверхурочныхВоеннослужащим");
...
Процедура ПодробнееОРасчетеУдержаний(Команда)
	...
	РасчетЗарплатыРасширенныйКлиент.ОткрытьФормуПодробнееОРасчетеУдержаний(Объект, "ОплатаСверхурочныхВоеннослужащим", ЭтаФорма, ОписаниеДокумента(ЭтаФорма));
...
Процедура ДополнитьФорму()
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.ГосударственнаяСлужба") Тогда
		...
		Модуль.ДокументыВыполненияНачисленийДополнитьФорму(ЭтаФорма, ОписаниеТаблицыСохраняемоеДенежноеДовольствие(), "Начисления", , Ложь);
		Модуль = ОбщегоНазначения.ОбщийМодуль("РасчетДенежногоДовольствия");
		Модуль.НастроитьДоступностьРедактированияПоказателейДенежногоДовольствия(ЭтаФорма, ОписаниеТаблицыНачислений());
	КонецЕсли;	
...
Процедура ВидРасчетаПриИзмененииНаСервере()
	...
	ЗарплатаКадрыРасширенный.ВводНачисленийВШапкеВидРасчетаПриИзменении(ЭтаФорма, ОписаниеТаблицыНачислений(), 2, Истина);
...
&НаСервере
Процедура ОбновитьДанныеДляРасчетаСохраняемогоДенежногоДовольствия(Сотрудники)
	
	СтруктураОбъекта = ПодготовитьСтруктуруОбъектаДляРасчетаДенежногоДовольствия(Сотрудники);
		
	РасчетДенежногоДовольствия.ОбновитьДанныеСохраняемогоДенежногоДовольствияДокумента(СтруктураОбъекта);
		
	РасчетДенежногоДовольствия.ЗаполнитьПоказательДенежноеДовольствиеСотрудников(ЭтаФорма, СтруктураОбъекта);
...
&НаСервере
Функция ПодготовитьСтруктуруОбъектаДляРасчетаДенежногоДовольствия(Сотрудники, НомерСтроки = Неопределено) Экспорт 
	...
	ОбъектСтруктура.Вставить("ПериодРасчета", 					Объект.ПериодРегистрации);	
	ОбъектСтруктура.Вставить("ДатаНачалаСобытия",				Объект.ПериодРегистрации);	
	ОбъектСтруктура.Вставить("ДенежноеДовольствие", 			Объект.ДенежноеДовольствие);
	ОбъектСтруктура.Вставить("Сотрудник", 						Сотрудники);
	ОбъектСтруктура.Вставить("УникальныйИдентификатор",			ЭтаФорма.УникальныйИдентификатор);	
	ОбъектСтруктура.Вставить("СохраняемоеДенежноеДовольствие", 	0);
	ОбъектСтруктура.Вставить("НомерТекущейСтроки", 				НомерСтроки);
...
Процедура НачисленияПослеУдаленияНаСервере(Сотрудник)
	...
	СтрокиДенежногоДовольствия = Объект.ДенежноеДовольствие.НайтиСтроки(Новый Структура("Сотрудник", Сотрудник));
	Для Каждого СтрокаДенежногоДовольствия Из СтрокиДенежногоДовольствия Цикл
		Объект.ДенежноеДовольствие.Удалить(СтрокаДенежногоДовольствия);
	КонецЦикла;
...
Процедура ОбработатьИзменениеСотрудника(ИдентификаторСтроки, ПрежнийСотрудник)
	...
	ОбновитьДанныеДляРасчетаСохраняемогоДенежногоДовольствия(ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(Сотрудник));
...
Процедура ОбработкаПодбораНаСервере(Знач Сотрудники)
	...
	ОбновитьДанныеДляРасчетаСохраняемогоДенежногоДовольствия(Сотрудники);
...
Процедура УстановитьВидимостьРасчетныхПолей()
	...
	УстановитьВидимостьПерерасчетов(ЭтаФорма);
    УстановитьВидимостьРеквизитовМежрасчетнойВыплаты();
	РасчетЗарплатыРасширенныйКлиентСервер.ДокументыВыполненияНачисленийУстановитьВидимостьПолейОтображенияВремени(ЭтаФорма, ОписаниеТаблицыНачислений());
